...
public class ValidationSupraUtil {
...
return CommonSupraConstants.STR_ZERO.equalsIgnoreCase(strValue);
...
!CommonSupraConstants.STR_QUESTION_MARK.equals(strVal.toString())
)){
...
valStr = CommonSupraUtil.getDefaultIfNull(valStr,null);
if(required && valStr == null){ExceptionSupraUtil.throwException(exceptionCode, ExceptionSupraUtil.EXCEPTION_VALIDATION);}
if(valStr != null && (valStr.length() < minLength || valStr.length() > maxLength)){
ExceptionSupraUtil.throwException(exceptionCode, ExceptionSupraUtil.EXCEPTION_VALIDATION);
}
return valStr;
}
...
SimpleDateFormat sdf = new SimpleDateFormat(CommonSupraConstants.DATE_ddMMyyyy);
